...
ARG pool_ip=192.168.178.10

RUN generate_indy_pool_transactions --nodes 4 --clients 5 --nodeNum 1 2 3 4 --ips="$pool_ip,$pool_ip,$pool_ip,$pool_ip"

COPY add-did.sh /usr/bin/add-did
COPY indy-cli-setup.sh /usr/bin/indy-cli-setup
COPY add-did-from-seed.sh /usr/bin/add-did-from-seed
COPY genesis/local-genesis.txn /etc/indy/genesis.txn
COPY indy-cli-config.json /etc/indy/indy-cli-config.json
